...
config = {
    ...
    appUrl: 'http://app_id_on_heroku.herokuapp.com',
    fbNamespace: 'facebookappnamespace',
            ...
            { parser: rest.parsers.json }
                ...
                rest.get(
                    ...
                    { parser: rest.parsers.json }
                        ...
                        var director = _.find(data[0].cast, function(c) { return c.job == 'Director' });
                        var writer = _.find(data[0].cast, function(c) { return c.job == 'Writer' });

                        movie.set({
                            genres:       _.map(data[0].genres, function(g) { return g.name }).join(', '),
                            studio:       _.map(data[0].studios, function(g) { return g.name }).join(', '),
